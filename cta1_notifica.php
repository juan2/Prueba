...
<?php
	...
	if (isset($_POST['enviar'])){
		$myfecha=Explode('-', $_POST['fecha1']);
		$fecha_ini=$myfecha[2].'-'.$myfecha[1].'-'.$myfecha[0];
		$fecha_ini_campo=$_POST['fecha1'];

		$myfecha=Explode('-', $_POST['fecha2']);
		$fecha_fin=$myfecha[2].'-'.$myfecha[1].'-'.$myfecha[0];
		$fecha_fin_campo=$_POST['fecha2'];
		$filtroxfecha=" and (date(fechaalta) BETWEEN '".$fecha_ini."' and '".$fecha_fin."')";

	}else{
		$fecha_ini=add_date(date('Y-m-d'),$day=-5,$mth=0,$yr=0);
		$fecha_ini_campo=cambiaDMA(add_date(date('Y-m-d'),$day=-5,$mth=0,$yr=0));

		$fecha_fin=date('Y-m-d');
		$fecha_fin_campo=date('d-m-Y');
		$filtroxfecha=" and leido=0";
	}
	...
	$statement = "ctra1_pras where eliminado=0 and id_unico in (select distinct idapoyo from ctra1_notifica where idusuario=".$_SESSION['MM_IdUsuario']." $filtroxfecha) $filtroxsecre_pag ";
...
#	$result99.=" LEFT JOIN ctra1_catalogos ct3 ON ct3.id_unico=pr.id_mpio ";
	...
	$result99.=" WHERE pr.eliminado=0 and pr.id_unico in (select distinct idapoyo from ctra1_notifica where idusuario=".$_SESSION['MM_IdUsuario']." $filtroxfecha) $filtroxsecre ";
...
<h3>Últimos Cambios Bitacora &nbsp;&nbsp;&nbsp;<i class="icon-asterisk"></i><?php echo $total_reg.' de '.$total_general; ?> registro(s)</h3>
	...
	<div class="accordion-group">
	  ...
	  <div id="collapseOne" class="accordion-body collapse">
		<div class="accordion-inner">
			...
			<div class="span5">
			<form class="form-inline" id="frmCaptura" name="frmCaptura" method="POST" action="cta1_notifica.php">
			<div class="row-fluid">
			<div class="span3">
				<fieldset>
				  ...
				  <div class="control-group">
					...
					<div class="controls docs-input-sizes">
					  <input class="span12" type="text" name="fecha1" id="fecha1" value="<?php echo $fecha_ini_campo; ?>" readonly>
					</div>
				  </div>
				  ...
				  <div class="control-group">
					...
					<div class="controls docs-input-sizes">
					  <input class="span12" type="text" name="fecha2" id="fecha2" value="<?php echo $fecha_fin_campo; ?>" readonly>
					</div>
				  </div>
